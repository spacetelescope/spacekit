...
from spacekit.preprocessor.transform import normalize_training_data, normalize_training_images
...
for train_idx, test_idx in kfold.split(X, y):
    ...
    img_idx, (X_tr, y_tr), (X_ts, y_ts), _ = training_img_aug(
        train, test, val=None
    )
    # normalization
...
for i in range(0, len(acc_per_fold)):
  print('--------'*11)
  print(f'> Fold {i+1} - Loss: {loss_per_fold[i]} - Accuracy: {acc_per_fold[i]}% - ROC: {np.round(roc_per_fold[i], 4)} - FP: {np.round((fp_per_fold[i]/T)*100, 4)} - FN: {np.round((fn_per_fold[i]/T)*100, 4)}')
print('--------'*11)
print('Averages:')
