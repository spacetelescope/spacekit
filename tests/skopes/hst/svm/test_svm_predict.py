...
def test_predict_alignment(svm_unlabeled_dataset, svm_pred_img, tmp_path):
    preds = predict_alignment(svm_unlabeled_dataset, svm_pred_img, output_path=tmp_path, size=128)
    assert len(preds) > 0
...
def test_load_predict(svm_unlabeled_dataset, svm_pred_img, tmp_path):
    ...
    preds = classify_alignments(X, model_path=None, output_path=tmp_path)
    assert len(preds) > 0
