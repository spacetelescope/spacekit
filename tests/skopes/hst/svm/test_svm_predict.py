...
def test_load_predict(svm_unlabeled_dataset, svm_pred_img, tmp_path):
    builder = Builder(blueprint="ensemble", model_path=None)
    model = builder.load_saved_model()
    builder.find_tx_file()
    X = load_mixed_inputs(
        svm_unlabeled_dataset, svm_pred_img, size=128, tx=builder.tx_file, norm=0
    )
    assert X[0].shape == (2, 10)
    assert X[1].shape == (2, 3, 128, 128, 3)

    preds = classify_alignments(X, model, output_path=tmp_path)
    assert len(preds) > 0
